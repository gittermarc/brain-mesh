...
> Start Here für neue Entwickler:innen. Stand: GRAPH-CHAT-RELATIONSHIP-INTENT-1 nach GRAPH-CHAT-NODE-PROFILE-PRESENTATION-1, GRAPH-CHAT-NODE-PROFILE-DOMAIN-1, GRAPH-CHAT-CORE-GROUNDING-1 und INTENT-COMPILER-7, mit vollständig lokaler Relationship-Intent-Familie für direkte Links, autoritativem app-owned Node-Profil, zentralem Mention Grounding, produktivem Typed-Planner-Cutover, gebündelter Limit-Policy, begrenzter Interpreter-Recovery und providerfreier lokaler Ausführung aller unterstützten Intent-Familien.
...
- **Semantic Intent Interpreter**: Nachgelagerte On-Device-Foundation-Models-Stufe für Find Nodes, Node Details, Compare Nodes, Inspect Graph State und direkte Relationships sowie fachliche Collection-, Filter-, Sortier-, Projektions-, Count-, Group- und Refinement-Bedeutung. Das Modell erhält weder Tools noch technische Identitäten und erzeugt nur einen begrenzten, untrusted und value-only Draft aus Anzeigenamen, fachlichen Relationen, Nutzerwerten, Graph-State-Aspekt und Conversation-Bezug.
...
- Erfolgreiche semantische Find-/Query-/Node-/Comparison-/Graph-State-/Relationship-Turns verwenden denselben normalisierten und live validierten Composable Read Plan als gemeinsame Ausführungssemantik und dahinter den Local Intent Execution Kernel, lokale Tools, Evidence Registry, Artifact Factory, Primary Result Ledger, deterministischen Answer Fallback, Presentation Firewall und Conversation Reducer. Es entsteht weder eine freie Answer-Provider-Session noch ein zweiter Modellaufruf.
...
4. konservativer providerfreier Relationship Fast Path;
5. nur andernfalls toolfreier Semantic Intent Interpreter;
6. Draft Validation;
7. appseitige Schema-, Scope- und Conversation-Auflösung;
8. fachliche Clarification oder vollständig gebundener Compiled Intent;
9. lokale Tool-/Query-Ausführung, Evidence-/Artifact-Commit und Finalisierung;
10. Legacy Provider ausschließlich für `unrecognized`, `openEnded` oder eindeutige technische Interpreter-Nichtverfügbarkeit.

Für Find Nodes, Entity List, Filtered List, Count, Group Count, Node Details, Refinement, Compare Nodes, Inspect Graph State und Relationships gilt damit: höchstens ein Semantic-Interpreter-Kontakt, kein Answer Provider, kein modellbestimmter Tool Call oder `GraphQueryPlan`, keine technischen Modell-Aliasse, kein modellbestimmtes Ergebnislimit und kein modellgenerierter finaler Fachtext. Foundational und Relationship Fast Paths besitzen null Modellkontakte. Interpretation Correction kompiliert aus der vertrauenswürdigen gebundenen Interpretation neu und verwendet weder Interpreter noch Provider.
...
### Graph Chat Composable Read Plan – gemeinsame lokale Read-Semantik

- `GraphChatComposableReadPlan.v1` ist value-only, `Hashable` und `Sendable`. Er bindet Graph-, Chat-, Query- und kompilierten Scope, Request/Conversation/Turn, Sprache, Typed-Intent-Familie, Query-Plan-Version, Reference Date, Result-/Evidence-/Artifact-Vertrag sowie Ergebnis-, Zwischen-, Operations- und Hop-Limits.
- Die lineare Operationsdomäne umfasst Entity-/Node-/Conversation-Result-Auswahl, Suche, autoritative Filter, Node-Identity-/Feldprojektion, stabile Sortierung, Count/Group Count, Node-Beschreibung, bestehende Vergleiche, gesicherten Graph State, direkte Relationship-Traversierung, Gegenknoten-/Richtungs-/Link-Notiz-Projektion und finales Limit. Jeder Step referenziert ausschließlich seinen unmittelbaren gebundenen Vorgänger.
- `GraphChatComposableReadPlanCompiler` wird nur von App-Adaptern nach fachlicher Typed-Intent-Compilation aufgerufen. Foundation Model, Semantic Draft und Correction UI enthalten weiterhin weder Plan-JSON noch IDs, Aliasse, Operatorumsetzung, Limits, Evidence- oder Artifact-Details.
...
- `GraphChatComposableReadPlanValidator` revalidiert Version, Bindings, vollständige Foundational-Aliasse, Field Owner, Node Owner, Scope Authorization, Filtertyp/Operator/Wert über `GraphQueryPlanValidator`, Projektion/Aggregation, Result-/Zwischenlimits, Evidence-/Artifact-Vertrag, direkte Traversierungsrichtung und Gegenendpunkte. `maximumTraversalHopCount` ist app-owned und exakt `1`; allgemeine Zwei-Hop-Ausführung bleibt gesperrt.
- Find Nodes, Entity Collection, Filtered List, Count, Group Count, Refinement, autoritatives Single Field, vollständige Node Details, Compare Nodes, Inspect Graph State und Relationships werden verlustfrei in den Plan kompiliert. `GraphChatTypedIntentAdaptation` speichert keine zweite Action-Wahrheit mehr; `action` ist nur noch eine berechnete Kompatibilitätssicht aus dem Read Plan.
- `GraphChatComposableReadPlanLegacyActionAdapter` ist bewusst temporär und eng: Er materialisiert die bereits vorhandenen Query-/Search-/Node-/Comparison-/Stats-/Relationship-Executor-Verträge aus einem validierten Plan, damit Result Normalization, Evidence, Artifacts, Result Windows, `CURRENT`, Ledger, Finalizer und Terminal Events unverändert bleiben. Er darf entfernt werden, sobald diese sechs Executor-Eingänge den Read Plan direkt konsumieren; bis dahin darf kein neuer fachlicher Sonderpfad in den Adapter aufgenommen werden.
- Lokale Artifact-Commit-Kontexte und Correction Origins tragen beziehungsweise prüfen `GraphChatComposableReadPlanVersion.current`. Ein älterer Kontext ohne Planversion ist nicht gleich einem aktuellen lokalen Commit-Kontext und kann keine editierbare Correction erzeugen. Jede Correction kompiliert nach frischer Schema-/Scope-/Checkpoint-Revalidation eine neue Planinstanz mit neuer Request-/Turn-Bindung.
- Die Planversion ist unabhängig von `GraphChatTypedIntentDomainVersion` und `GraphQueryPlan.currentVersion`. Eine Änderung an Operationsbedeutung, Normalisierung oder Result-/Artifact-Vertrag verlangt eine neue Read-Plan-Version und eine explizite Checkpoint-/Artifact-Kompatibilitätsentscheidung; inkompatible alte lokale Sessions werden niemals still weiterverwendet.
