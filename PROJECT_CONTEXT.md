...
- **Interpretation Correction**: Versionierter, an ursprüngliche User-/Assistant-Message, normalisierte Request-Frage, Request, Turn, Conversation, Graph-/Chat-Scope, Checkpoints, Intent-Domainversion und Artifact-Session gebundener Ersatzturn. Die UI-Auswahl ist nur Nutzerabsicht; ein frischer vollständiger `GraphSchemaContext` und der bestehende Query-/Advanced-/Relationship-Intent-Compiler revalidieren alle Identitäten vor der providerfreien lokalen Ausführung.
...
- Erfolgreiche semantische Find-/Query-/Node-/Comparison-/Graph-State-/Relationship-Turns verwenden den gemeinsamen Local Intent Execution Kernel, lokale Tools, Evidence Registry, Artifact Factory, Primary Result Ledger, deterministischen Answer Fallback, Presentation Firewall und Conversation Reducer. Es entsteht weder eine freie Answer-Provider-Session noch ein zweiter Modellaufruf.
