...
- Produktions-Swift: 568 Dateien, 93.133 Zeilen unter `BrainMesh/`.
...
- **Graph Chat Domain**: `BrainMesh/GraphChat/` enthält die releasefähige read-only Stufe 1 mit promptfähigen, begrenzten Schema-Snapshots ohne rohe UUIDs, appseitiger Alias-Auflösung, strikt validierten Query-Plänen, deterministischer Query Engine, sechs read-only Tools, revalidierter Evidence und einer value-only Answer-Artifact-Domain. Strukturierte Tool-Ergebnisse können als typisierte, sessionlokale Artifacts für Kennzahlen, Ergebnislisten, Tabellen, Rankings, Gruppierungen, Vergleiche, Health Findings und Timelines registriert werden. Der Provider sieht ausschließlich opaque IDs aus aktuellen Tool-Aufrufen und darf Payloads weder erzeugen noch verändern. `GraphChatModelProvider` kapselt ausschließlich Apples on-device Foundation Models; `GraphChatOrchestrator` bindet exakt einen aktiven Graph-/Chat-Scope, zentrale Tool-/Result-/Evidence-/Artifact-Budgets, Streaming, Cancellation, einen flüchtigen und streng begrenzten strukturierten Conversation State sowie finale Source-Validierung zusammen. Multi-Turn-Folgefragen erhalten ausschließlich einen separaten, budgetierten Provider-Snapshot aus validierten Aliasen, stabilen Ergebnisreihenfolgen, Entity-/Feld-/Gruppenbezügen, der letzten validierten Query-Struktur und technischen Turn-Zusammenfassungen. Typisierte Referenzvorschläge des Modells werden vor jeder Tool- oder Query-Ausführung zentral gegen Graph, Scope, Entity, Ordinalgrenzen, aktuelle Repository-Daten und erneut ausgeführte Query-Ergebnisse validiert. Mehrdeutigkeiten erzeugen eine typisierte Pending-Clarification mit begrenzten Optionen; Auswahl, Fortsetzung und Lifecycle-Reset bleiben atomar im bestehenden Conversation-State-Reducer. Antwort, Klärung, No-Result und Unsupported sind eigene Domain-Zustände, und die Antwortsprache wird pro aktueller Nutzerfrage deterministisch als Deutsch oder Englisch gewählt. Der Conversation State entsteht ausschließlich aus appseitig validierten Query-Plänen, revalidierten Tool-Ergebnissen und technischen Abschlussmetadaten; ungeprüfter Assistant-Text wird nicht übernommen und ein Turn wird erst nach erfolgreichem Abschluss atomar committed. `GraphChatAccessPolicy` ist die einzige deterministische Produktentscheidung für aktiven/existierenden Graphen, Lock, Free/Unknown/Pro, Modellverfügbarkeit, Index, Reconciliation und laufende Generation; das zusätzliche Execution Gate erzwingt dieselbe Autorisierung unmittelbar vor jedem Modellaufruf. `GraphChatLaunchCoordinator` und `GraphChatSessionStore` integrieren Root-Tab, Command Center, Entity-/Attribute-Details, Graph Canvas und Graph-Health-Befunde, halten Draft und Verlauf ausschließlich in Memory und verwerfen Sessiondaten bei Lock, Background, Graphwechsel, Import/Replace/Delete sowie App-Terminierung. Ein vor dem Paywall erfasster Draft bleibt im Speicher, wird nach Kauf und erneuter Runtime-Prüfung aber nie automatisch gesendet. Finale Antworten zeigen revalidierte Quellen und die von der deterministischen Query Engine tatsächlich angewendeten Filter; Attachment-Inhalte werden weder gelesen noch behauptet. `BMLog.chat` protokolliert ausschließlich Dauer, Counts, Tooltypen, Index-Fallback, Availability, Cancellation und technische Fehlercodes. Cloud-Provider, Attachment-Inhaltsanalyse und Schreiboperationen sind nicht enthalten.
...
- `BrainMesh/GraphChat/`
  ...
  - `UI/` enthält die produktive, dependency-injizierte SwiftUI-Präsentationsschicht mit Main-Actor-ViewModel, Composer samt explizitem Tastatur-Schließen, differenzierter Pro-/Availability-/Index-/Reconciliation-UX, deterministischer Stream-Reduktion, navigation-agnostischen Evidence-Karten und ausschließlich flüchtigem graph-/scope-getrenntem Verlauf. Der Root-Tab und alle kontextuellen Einstiege verwenden dieselbe Policy und denselben Session Store.

### Graph Chat MVP-Grenzen und spätere Stufen

- Stufe 1 ist ausschließlich read-only, on-device und auf den aktiven entsperrten Graphen begrenzt. Große Graphen werden über lokalen Index, Schema-Kompaktion und Tool-Budgets verarbeitet; ein vollständiger Graph wird nicht in einen Modellprompt serialisiert.
- Die Answer-Artifact-Domain und Registry sind vorhanden, eine vollständige neue Artifact-UI ist bewusst noch nicht Teil dieser Stufe. Textantworten und bestehende Evidence-Karten bleiben der produktive Fallback.
- Optionaler Cloud-Fallback, semantisches Reranking oder Embeddings, Attachment-Textextraktion, schreibende Tools mit Vorschau und Bestätigung, Graph-Pfad-Artifacts sowie Multi-Hop-Pfadsuche sind ausdrücklich spätere Stufen.
