...
- `BrainMesh/GraphChat/`
  ...
  - `Orchestration/` initialisiert Schema, Provider, request-scoped Tool-Runner, Budget, Evidence-Registry und Conversation-State-Transaktion, verwirft Sessions und offene Klärungen bei Graph-/Scope-/Lifecycle-Wechsel, erlaubt höchstens eine laufende Generation und committed State-Kandidaten erst nach finaler Evidence-Revalidierung atomar. Lokale Referenzauflösung und Unsupported-Erkennung laufen vor der Modell-Session; eine gültige Klärungsauswahl setzt die ursprüngliche Operation mit dem revalidierten `CURRENT`-Alias fort.
...
- Optionaler Cloud-Fallback, semantisches Reranking oder Embeddings, Attachment-Textextraktion, schreibende Tools mit Vorschau und Bestätigung sowie Multi-Hop-Pfadsuche sind ausdrücklich spätere Stufen.
