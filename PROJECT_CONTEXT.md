...
| `BrainMesh/GraphChat/` | Chat-Kernmodelle, Schema-Snapshot, Query-Plan-Validierung, deterministische Query Engine, read-only Tools, Evidence, Foundation-Models-Provider und Streaming-Orchestrator | UI-unabhängig; der Provider ist austauschbar, produktiv on-device; nur kontrollierte read-only Tools erreichen die Faktenebene; partielle Antworten enthalten keine aktivierten Quellen, finale Sources werden graph- und scope-scoped gegen die Request-Registry validiert; keine Attachment-Inhalte oder Writes |
