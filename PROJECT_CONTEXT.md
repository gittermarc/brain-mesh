...
- Produktions-Swift: 561 Dateien, 87.952 Zeilen unter `BrainMesh/`.
...
- `BrainMesh/GraphChat/`
  ...
  - `Provider/` kapselt Availability, Sessions, Foundation-Models-Tool-Calling, Guided Generation und providerunabhängige Stream-Ereignisse. Die produktive Implementierung verwendet ausschließlich `SystemLanguageModel.default`; Tests verwenden einen vollständig deterministischen Fake-Provider.
  - `Conversation/` definiert den value-only, `Sendable`, graph-/scope-isolierten und zentral budgetierten Conversation State samt einzigem trusted Reducer und request-scoped Transaktion. Der State bleibt ausschließlich in Memory und enthält weder Nutzerfragen noch Assistant-Antworttexte.
  - `Orchestration/` initialisiert Schema, Provider, request-scoped Tool-Runner, Budget, Evidence-Registry und Conversation-State-Transaktion, verwirft Sessions bei Graph-/Scope-Wechsel, erlaubt höchstens eine laufende Generation und committed State-Kandidaten erst nach finaler Evidence-Revalidierung atomar.
