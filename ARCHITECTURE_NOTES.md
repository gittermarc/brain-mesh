...
- Umgesetzt unter `BrainMesh/GraphChat/`:
  ...
  - Partielle Stream-Inhalte tragen keine navigierbaren Quellen oder Artifacts. Erst die finale strukturierte Antwort wird gegen die request-scoped `GraphChatEvidenceRegistry` und die aktuelle Artifact-Transaktion geprüft; unbekannte IDs werden entfernt, tatsächlich ausgeführte Query-Filter werden deterministisch dedupliziert und eine Antwort ohne validierte Evidence wird als unzureichend gekennzeichnet. Bestehender Textchat bleibt vollständig kompatibel, wenn ein Tool kein Artifact erzeugt, Artifact-Staging verworfen wird oder die UI noch keine Artifact-Darstellung anbietet.
  - `UI/` setzt die SwiftUI-Präsentationsschicht auf diese Grenzen: `GraphChatViewModel` ist Main-Actor-isoliert, hält höchstens eine Generation, übersetzt Stream-Ereignisse deterministisch in laufende, partielle, finale, No-Results-, Availability-, Fehler-, Retry- und Cancellation-Zustände und speichert den Verlauf ausschließlich graph-/scope-getrennt im injizierten In-Memory-Store. Composer, schemaabhängige Vorschläge, differenzierte Modell-/Index-/Reconciliation-Zustände und Evidence-Karten sind getrennte Komponenten; der Composer besitzt eine explizite Tastatur-Schließen-Aktion. Navigation wird nur über reine injizierte Actions ausgelöst.
  ...
  - `GraphChatSessionStore` beobachtet `GraphMutationEventBus`. Import, Replace, Delete und Full-Rebuild des aktiven Graphen markieren den Index stale und verwerfen Generation, Provider-Session, Evidence, Verlaufssicht und Composer-State; graphfremde Events werden ignoriert. Graphwechsel, spezifischer/globaler Lock, Background, Entitlement-Verlust und App-Terminierung führen denselben in-memory Cleanup aus.
...
- Bewusste MVP-Grenzen und spätere Stufen:
  - Vollständige Artifact-Rendering-UI und Presentation-Lokalisierung.
  - Optionaler Cloud-Fallback.
  - Semantisches Reranking oder Embeddings.
  - Attachment-Textextraktion.
  - Schreibende Tools ausschließlich mit späterer Vorschau und expliziter Bestätigung.
  - Graph-Pfad-Artifacts und Multi-Hop-Pfadsuche.
