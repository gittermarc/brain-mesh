...
- Umgesetzt unter `BrainMesh/GraphChat/`:
  ...
  - `GraphChatOrchestrator` erzeugt pro Anfrage eine frische, graph-/scope-gebundene Provider-Session, erlaubt genau eine laufende Generation, verwirft vorbereitete Sessions und offene Klärungen bei Graph-/Scope-Wechsel, neuem Chat, Lock oder Access-Reset und folgt der Policy „neue Anfrage cancelt und wartet auf die vorherige“. Jeder Turn verwendet einen unveränderlichen Conversation-State-Snapshot; Tool-Ereignisse reduzieren nur einen request-scoped Kandidaten, der erst nach erfolgreicher Abschlussantwort und finaler Evidence-Revalidierung atomar übernommen wird. Fehler und Cancellation behalten den vorherigen State. Eine gültige Klärungsauswahl setzt ausschließlich die ursprüngliche Operation mit dem appseitig revalidierten `CURRENT`-Alias fort. Ein Context-Window-Fehler darf genau einen Retry mit frischer Session, zurückgesetzter State-Transaktion und weiter reduziertem Provider-Kontext auslösen.
  ...
  - Partielle Stream-Inhalte tragen keine navigierbaren Quellen. Erst die finale strukturierte Antwort wird gegen die request-scoped `GraphChatEvidenceRegistry` geprüft; unbekannte IDs werden entfernt, tatsächlich ausgeführte Query-Filter werden dort deterministisch dedupliziert und eine Antwort ohne validierte Evidence wird als unzureichend gekennzeichnet.
