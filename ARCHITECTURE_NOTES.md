...
- Produktionscode unter `BrainMesh/`: 561 Swift-Dateien, 87.952 Zeilen.
...
- `BrainMesh/ContentView.swift`
  ...
  - Tabs: `EntitiesHomeView`, `GraphCanvasScreen`, `GraphChatTabView`, `GraphStatsView`, `SettingsView` in `NavigationStack`. Auf regulären iPads kann `GraphCanvasScreen` denselben `GraphChatTabView` zusätzlich als adaptiven Inspector neben dem bestehenden Canvas hosten; iPhone bleibt beim vorhandenen Tab-/Navigation-Flow.
...
#### Graph Chat Read-only Copilot Boundary
...
- Umgesetzt unter `BrainMesh/GraphChat/`:
  ...
  - `Provider/` definiert `GraphChatModelProvider` mit UI-unabhängiger Availability-, Session- und Streaming-Domain. `FoundationModelsGraphChatProvider` mappt `SystemLanguageModel.default.availability`, verwendet `LanguageModelSession` mit klaren Instructions, exakt sechs Foundation-Models-`Tool`-Adaptern und `@Generable` für die strukturierte Abschlussantwort. Ein Fake-Provider erlaubt vollständige Tests ohne Modellinferenz.
  - Die Foundation-Models-Adapter akzeptieren ausschließlich kompakte Entity-, Field- und Node-Aliase sowie begrenzte Argumente. Graph- und Chat-Scope stammen unveränderbar aus der appseitigen Session-Konfiguration; jeder Aufruf läuft erneut durch die bestehenden Validatoren, deterministischen Tools und das zentrale Request-Budget. Tool-Ergebnisse enthalten ausschließlich begrenzte value-only Inhalte, registrierte Evidence-IDs und optional genau eine opaque Artifact-ID. Der Provider darf ausschließlich eine von einem aktuellen Tool-Aufruf erhaltene ID unverändert in die Abschlussantwort übernehmen; Payload-Erzeugung und -Mutation liegen vollständig außerhalb seiner Autorität.
  - `Artifacts/` modelliert Kennzahl, Ergebnisliste, Tabelle, Ranking, Gruppierung, Vergleich, Health Finding und Timeline als value-only, `Hashable` und `Sendable`. Zellen und Werte bleiben typisiert (`text`, `integer`, `decimal`, `boolean`, `date`, `dateTime`, `duration`, `percentage`, `choice`, `missing`); lokalisierte Stringdarstellung ist keine Aufgabe der Domain. Evidence kann auf Artifact-, Payload-/Abschnitts-, Zeilen- und optional Wertebene gebunden werden. Navigation Targets enthalten ausschließlich graph-scoped IDs und value-only Filter, niemals Closures, Repositories oder `ModelContext`.
  - `GraphChatAnswerArtifactRegistry` ist actor-isoliert, session- und graphgebunden sowie ausschließlich in Memory. Tool-Läufe stagen Artifacts in einer request-scoped Transaktion erst nach vollständiger Evidence- und Navigation-Revalidierung. Die Abschlussantwort darf nur IDs derselben aktuellen Transaktion übernehmen; erfundene, fremde, frühere oder nicht registrierte IDs werden entfernt. Erst der erfolgreiche Conversation-State-Abschluss committed die ausgewählten Artifacts. Cancellation, Fehler und Context-Retry rollen die Transaktion zurück; neuer Chat, Graph-/Scope-Wechsel, Lock, Delete, Access-Reset und Checkpoint-Restore leeren die Registry. Deterministische oldest-first Eviction begrenzt Anzahl und Byte-Budget.
  - `Conversation/` enthält `GraphChatConversationState`, zentrale Budgets, auflösbare Node-/Entity-/Field-/Result-/Group-Referenzen, begrenzte Turn-Kontexte und den einzigen trusted Reducer. `GraphChatConversationContextBuilder` erzeugt daraus einen separaten, hart begrenzten Provider-Snapshot mit stabilen lokalen Aliasen, letzter validierter Query-Struktur, begrenzten Ergebnis-/Gruppenreferenzen und technischen Turn-Zusammenfassungen. Rohverlauf, ungeprüfter Assistant-Text und ungeprüfte Modell-IDs werden nicht übertragen. Die lokale Referenzerkennung verwendet tokenisierte Wortgrenzen und explizite Ergebnisphrasen; Demonstrativwörter ohne eindeutigen Follow-up-Cue lösen keinen Referenzpfad aus. Kann eine plausible implizite Referenz mangels lokalem State nicht aufgelöst werden und existieren keine validierten Klärungsoptionen, darf der Orchestrator die aktuelle Frage regulär an den Provider weitergeben. Liefert der Provider anschließend eine verwendbare Antwort zusammen mit einem nicht auflösbaren Missing-Context-Referenzvorschlag, wird nur der unverbindliche Vorschlag verworfen; die Antwort darf nicht in eine nachträgliche lokale Klärung umgewandelt werden. Eine offene `GraphChatPendingClarification` enthält ausschließlich die typisierte Entscheidung, maximal acht gültige Optionen, Ursprungsturn, Graph, Scope, Fortsetzungsoperation, eine begrenzte nicht-faktische Fortsetzungsfrage und Ablaufzeit; die Fortsetzungsfrage gehört nicht zum allgemeinen Provider-Snapshot.
  - `GraphChatConversationReferenceResolver` ist die einzige Multi-Turn-Auflösungsgrenze. Modellvorschläge für letzte Ergebnismenge, Teilmenge, Ordinalposition, Entity, Feld, Gruppe, Node oder Vergleich werden vor jeder Tool-/Query-Ausführung gegen Graph, Scope, Entity, Ordinalgrenzen, Repository-Existenz und – für Query-Resultate – eine erneute Ausführung des letzten validierten Plans geprüft. Geänderte Anzahl, Reihenfolge oder Gruppenzusammensetzung macht die Referenz stale; es gibt keinen stillen Fallback auf eine geratene Interpretation.
  - `GraphChatOrchestrator` erzeugt pro Anfrage eine frische, graph-/scope-gebundene Provider-Session, verwendet daneben genau eine sessiongebundene Artifact-Registry und erlaubt genau eine laufende Generation. Vorbereitete Sessions, staged Artifacts und offene Klärungen werden bei Graph-/Scope-Wechsel, neuem Chat, Lock oder Access-Reset verworfen; die Policy bleibt „neue Anfrage cancelt und wartet auf die vorherige“. Jeder Turn verwendet einen unveränderlichen Conversation-State-Snapshot; Tool-Ereignisse reduzieren nur request-scoped State- und Artifact-Kandidaten, die erst nach erfolgreicher Abschlussantwort sowie finaler Evidence- und Artifact-ID-Revalidierung atomar übernommen werden. Fehler und Cancellation behalten den vorherigen State. Eine gültige Klärungsauswahl setzt ausschließlich die ursprüngliche Operation mit dem appseitig revalidierten `CURRENT`-Alias fort. Schema, Conversation-Snapshot und Frage werden über `GraphChatModelContextProfile` als Standard-, Compact- oder Recovery-Prompt begrenzt; strukturierte Tool-Ausgaben besitzen ein separates zentrales Zeichenbudget. Ein Context-Window-Fehler darf genau einen Retry auslösen. Dieser Recovery-Lauf erhält eine neue Provider-Session, ein neues engeres Tool-Budget, eine neue Evidence-Registry, eine neue Artifact-Transaktion, eine neue Conversation-State-Transaktion und einen nochmals reduzierten Provider-Snapshot; partielle Tool-Ergebnisse des ersten Versuchs dürfen weder State noch Artifacts des Recovery-Laufs beeinflussen.
  - `GraphChatAnswerState` unterscheidet normale Antwort, Klärungsfrage, No-Result und Unsupported unabhängig vom sichtbaren Text. Technische Fehler bleiben ein eigener Stream-/UI-Zustand. `GraphChatResponseLanguageSelector` entscheidet Deutsch oder Englisch pro aktueller Nutzerfrage mit deterministischem App-Sprach-Fallback; Systeminstruktionen, Provider-Prompt und lokale Fallbacks verwenden dieselbe Turn-Sprache.
  - Partielle Stream-Inhalte tragen keine navigierbaren Quellen oder Artifacts. Erst die finale strukturierte Antwort wird gegen die request-scoped `GraphChatEvidenceRegistry` und die aktuelle Artifact-Transaktion geprüft; unbekannte IDs werden entfernt, tatsächlich ausgeführte Query-Filter werden deterministisch dedupliziert und eine Antwort ohne validierte Evidence wird als unzureichend gekennzeichnet. Actions auf noch nicht committed Artifacts existieren nicht. Der Textchat bleibt vollständig kompatibel, wenn ein Tool kein Artifact erzeugt oder Artifact-Staging verworfen wird.
  - `UI/` setzt die SwiftUI-Präsentationsschicht auf diese Grenzen: `GraphChatViewModel` ist Main-Actor-isoliert, hält höchstens eine Generation, übersetzt Stream-Ereignisse deterministisch in laufende, partielle, finale, No-Results-, Unsupported-, Availability-, Fehler-, Retry- und Cancellation-Zustände und speichert den Verlauf ausschließlich graph-/scope-getrennt im injizierten In-Memory-Store. Die graphnative Answer-UI rendert typisierte Tabellen, Rankings, Gruppierungen, Vergleiche, Health Findings und Timelines, ergänzt Evidence Chips und öffnet den Evidence Drawer mit revalidierter Query Summary. Große Reihenmengen verwenden Lazy-Container und zentrale Darstellungsgrenzen.
  - `Routing/` stellt mit `GraphChatLaunchCoordinator` value-only Scope-/Prompt-/Presentation-Requests für Root-Tab, Command Center, Entity-/Attribute-Details, Canvas-Auswahl und Stats-Befunde bereit. Alle Einstiege verwenden dieselbe `GraphChatView` und denselben `GraphChatSessionStore`. Ein vor dem Paywall erfasster Draft bleibt ausschließlich in Memory; nach Kauf und erneuter Availability-/Index-Prüfung ist eine sichtbare Nutzeraktion zum Senden erforderlich.
  - `Workspace/GraphCopilotWorkspaceCoordinator` ist die einzige flüchtige Brücke zwischen Chat und Canvas. Er übernimmt keinen Canvas State und keinen Selection Store, sondern publiziert graph-sichere Einmal-Commands an den bestehenden `GraphCanvasSelectionState`: fokussieren, hervorheben, Highlight entfernen, zur Auswahl hinzufügen und Auswahl ersetzen. Result Sheets sind auf bereits sicher geladene Nodes und `GraphChatWorkspaceBudget` begrenzt. Persistiert werden ausschließlich Inspector-Sichtbarkeit und bevorzugte Breite.
  - Auf regulären iPads hostet `GraphCanvasScreen` denselben produktiven `GraphChatTabView` als adaptiven Inspector neben derselben Canvas-Instanz. Auf iPhone bleiben Root-Tab, Graph-Jump und bestehende Detail-/Ergebnisnavigation unverändert. Das Schließen eines einzelnen Hosts cancelt keinen Stream, solange derselbe Chat noch in einem anderen Host sichtbar ist; der letzte Host beendet nur die laufende Operation und verwirft keinen bereits committed Verlauf.
  - Die Canvas-Auswahl wird als sichtbarer Scope-Vorschlag gespiegelt. Erst „Auswahl als Chat-Kontext übernehmen“ erzeugt den Selection-Scope; „Gesamter Graph“ macht dies rückgängig. Eine Auswahländerung während Streaming verändert weder Graph noch Scope des laufenden Turns, weil dessen Snapshot beim Start unveränderlich gebunden ist. Leere, gelöschte oder graphfremde Selection-Referenzen werden nicht in den Conversation State übernommen.
  - `Security/GraphChatAccessPolicy` ist die einzige Produktentscheidung für gültigen aktiven Graphen, Graph-Existenz, exakten Scope, Lock, Unknown/Free/Pro, Foundation-Models-Availability, Indexzustand, Reconciliation und laufende Generation. Root-/Kontext-Einstiege und Composer konsumieren dieselbe Entscheidung. Das separate `GraphChatExecutionGate` bleibt als Defense in Depth unmittelbar vor dem Orchestrator bestehen.
  - `GraphChatSessionStore` beobachtet `GraphMutationEventBus`. Import, Replace, Delete und Full-Rebuild des aktiven Graphen markieren den Index stale und verwerfen Generation, Provider-Session, Evidence, Verlaufssicht, Composer, Artifacts und Workspace-Präsentation; graphfremde Events werden ignoriert. Sonstige Mutationen des aktiven Graphen erhöhen eine Präsentationsrevision, durch die sichtbare Evidence und Artifacts erneut gegen aktuelle Repository-Daten aufgelöst werden. Graphwechsel, spezifischer/globaler Lock, Background, Entitlement-Verlust und App-Terminierung führen denselben graph-sicheren In-Memory-Cleanup aus.
  - Evidence- und Artifact-Actions werden ausschließlich für finale committed Antworten aktiviert und unmittelbar vor Ausführung revalidiert. Entity/Node öffnen bestehende Details, Filter und begrenzte Ergebnislisten verwenden vorhandene Navigation, Sources können fokussiert und Result Nodes flüchtig hervorgehoben werden, und Add/Replace wirken nur auf die bestehende Canvas-Auswahl. „Diese Nodes vergleichen“ erzeugt ausschließlich die bereits unterstützte strukturierte Detailvergleichsanfrage; Query Plan v2 und Multi-Hop werden nicht eingeführt.
  - Der Chat-Tab und `ProCenterView` führen `ProFeature.chatWithGraph` als klar erkennbares Pro-Feature. Free und Unknown starten weder Modell noch Tools. Die Free-Preview darf ausschließlich schema-abgeleitete Fragen für einen entsperrten aktiven Graphen anzeigen. Die Paywall beschreibt korrekt, dass ein Pro-Entitlement keine Foundation-Models-Verfügbarkeit auf jedem iOS-26-Gerät garantiert.
  - Trust-UX zeigt finale validierte Sources und angewendete Filter. No-Results benennt ausdrücklich, dass nur dokumentierte Graphdaten berücksichtigt wurden; interpretierbare Begriffe wie „wichtig“ müssen durch konkrete angewendete Filter erklärt werden. Attachment-Tools liefern Metadaten mit explizitem `contentNotRead`, niemals Binär- oder Dateiinhalte.
  - `Observability/GraphChatObservability.swift` und `BMLog.chat` erfassen ausschließlich End-to-End-Dauer, Tool-Count/-Typen, Evidence-Count, Index-Fallback, Availability, Cancellation, Outcome und technische Fehlercodes. Prompt, Antwort, Notes, Detailwerte, Dateinamen und Attachment-Inhalte sind nicht Teil des Metric-Modells oder der Logs.
  - Die produktive Integration bleibt vollständig on-device, verwendet keinen API-Key und keinen eigenen Backend- oder Cloud-Fallback. Das SwiftData-/CloudKit-Hauptschema bleibt unverändert.
- Bewusst verschobene Themen nach Abschluss der read-only Stufen 1 bis 3:
  - Query Plan v2.
  - Multi-Hop- und Graph-Pfad-Abfragen.
  - Persistierte Threads oder Chatverläufe.
  - Graph-Mutationen beziehungsweise schreibende Chat-Tools.
  - Attachment-Inhaltsanalyse oder Textextraktion.
  - Semantische Retrieval-Schicht, Embeddings oder Reranking.
  - Cloud-Provider oder Backend-Fallback.
