...
/// Test-only characterization of the physics calculation that previously lived
/// in `GraphCanvasView.stepSimulation()`.
enum LegacyGraphPhysicsReference {
    static func step(
        ...
        for i in 0..<simulatedNodes.count {
            ...
            for j in (i + 1)..<simulatedNodes.count {
                ...
                let force = configuration.repulsion / distanceSquared
                let repulsionX =
                    dx * force * configuration.repulsionScale
                let repulsionY =
                    dy * force * configuration.repulsionScale
                addVelocity(
                    firstKey,
                    dx: repulsionX,
                    dy: repulsionY,
        ...
        var maximumSimulatedSpeed: CGFloat = 0
        ...
        for node in simulatedNodes {
            ...
            if clampedSpeed > maximumSimulatedSpeed {
                maximumSimulatedSpeed = clampedSpeed
            }
        ...
        return GraphPhysicsStepResult(
            positions: positions,
            velocities: velocities,
            maxSimSpeed: maximumSimulatedSpeed,
    ...
    private static func addVelocity(
        ...
        guard !input.fixedNodeKeys.contains(key) else { return }
