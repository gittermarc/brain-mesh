...
enum IconCatalog {
    ...
    /// Precomputed search index (case-/diacritic-insensitive).
    /// We also normalize separators ('.', '-', '_') to spaces so searches like
    /// "person crop" can match "person.crop".
    static let searchIndex: [(symbol: String, key: String)] = {
        allSymbols.map { symbol in
            (symbol: symbol, key: makeSearchKey(symbol))
        }
    }()

    static func search(term: String, limit: Int = 360) -> [String] {
        let keyTerm = makeSearchKey(term)
        ...
        if prefixMatches.count >= limit { return Array(prefixMatches.prefix(limit)) }
